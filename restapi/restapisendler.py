...
class JSONSender:
    ...
    @staticmethod
    def send_data(
        host: str, port: int, endpoint: str, data: dict, output_file: str
    ) -> None:
        """
        Send JSON data to the specified API endpoint and save the response or error to a file.

        :param str host: The host address of the API server.
        :param int port: The port number of the API server.
        :param str endpoint: The API endpoint (e.g., 'insert', 'search', 'delete').
        :param dict data: The JSON data to send.
        :param str output_file: Path to save the server's response or error details.
        """
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input_file", help="The JSON file from which the data is received"
    )
    parser.add_argument(
        "--output_file",
        default="response.json",
        help="The file to save server response",
    )
...
def main() -> None:
    ...
    try:
        ...
        JSONSender.send_data(
            args.host, args.port, args.endpoint, data, args.output_file
        )
