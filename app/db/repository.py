...
class ClickHouseRepository:
        ...
        try:
            async with await self.connection.get_cursor() as cursor:
                await cursor.execute(
                    Queries.CREATE_DATABASE.format(database=self.database)
                )
    ...
    async def create_table(
        self, table_name: str, id_column: str, vector_column: str
    ) -> None:
        """
        ...
        try:
            async with await self.connection.get_cursor() as cursor:
                ...
                logging.info(
                    f"Table '{table_name}' created successfully in database '{self.database}'."
                )
        except errors.Error as e:
    ...
    async def ensure_db_and_table(
        ...
        try:
            if not await self.check_db_exists():
                logging.warning(
                    f"Database '{self.database}' does not exist. Creating..."
                )
